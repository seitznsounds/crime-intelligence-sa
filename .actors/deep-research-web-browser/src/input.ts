...
function validateAndFillInput(input: Partial<Input>, standbyInit: boolean): Input {
    ...
    // Recursive depth
    input.recursiveDepth = validateRange(
        input.recursiveDepth,
        inputSchema.properties.recursiveDepth.minimum,
        inputSchema.properties.recursiveDepth.maximum,
        inputSchema.properties.recursiveDepth.default,
        'recursiveDepth',
    );

    // Detailed analysis
    if (input.detailedAnalysis === undefined) {
        input.detailedAnalysis = inputSchema.properties.detailedAnalysis.default;
    }
