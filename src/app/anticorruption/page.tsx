    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-20">
        {[
          ...
          { label: "Reporting Void",  value: "15%",      sub: "Aware but won't report",  icon: <EyeOff className="w-5 h-5 text-accent-gold" /> },
          { label: "Fear Multiplier", value: "62%",      sub: "Fear retaliation if reporting",     icon: <Lock className="w-5 h-5 text-accent-crimson" /> },
          { label: "Zondo Progress",  value: `${enacted}/10`, sub: "Pillars enacted into law",       icon: <Scale className="w-5 h-5 text-accent-blue" /> },
        ].map((kpi, i) => (
          <motion.div
            key={i}
                    ...
                    {SOCIAL_NORM_METRICS.map((m, i) => (
                      <div key={i} className="p-4 bg-bg-glass border border-border-glass rounded-xl text-nowrap overflow-hidden">
                        <div className="flex justify-between items-start mb-3">
                          <div className="flex items-center gap-2">{m.icon}<span className="text-[9px] font-bold uppercase tracking-wider text-muted-foreground truncate max-w-[120px]">{m.label}</span></div>
