      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="glass-card p-6 border-border-glass bg-bg-glass">
            <h2 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground mb-6 flex items-center justify-between">
              Threat Matrix
            ...
            <div className="flex items-center gap-6 mb-8">
              <div className="relative w-20 h-20 flex items-center justify-center">
                <svg className="w-full h-full transform -rotate-90">
                  <circle cx="40" cy="40" r="36" stroke="currentColor" strokeWidth="4" fill="transparent" className="text-foreground/[0.03]" />
                  <circle 
                    cx="40" cy="40" r="36" 
                ...
                <span className="absolute text-xl font-bold font-mono tracking-tighter text-foreground">{person.risk_score || '??'}%</span>
              </div>
              <div>
                <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold mb-1">Risk Evaluation</p>
        ...
        <div className="lg:col-span-8 space-y-6">
          <header className="mb-4">
            ...
            <h1 className="text-5xl font-bold tracking-tighter mb-1 uppercase text-foreground">{person.full_name}</h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
              {person.metadata ? (
                Object.entries(person.metadata).flatMap(([key, value]) => {
                  ...
                  <div key={k} className="flex justify-between items-center py-1 border-b border-border-glass text-[10px] font-mono">
                    <span className="text-muted-foreground capitalize">{k.replace(/_/g, ' ')}</span>
                    <span className="text-accent-blue/80 truncate max-w-[140px] text-right">{String(v)}</span>
                  </div>
                ))
              ) : (
                <p className="col-span-2 text-[10px] text-muted-foreground italic">No additional metadata indexed.</p>
              )}
            </div>
          </div>
          ...
          <section className="glass-card p-8 bg-bg-glass-heavy">
            ...
            <div className="space-y-12 relative before:absolute before:left-3 before:top-2 before:bottom-2 before:w-[1px] before:bg-border-glass">
              {exposures && exposures.length > 0 ? (
                exposures.map((exp) => (
                  <div key={exp.id} className="relative pl-10">
                    <div className="absolute left-0 top-1.5 w-6 h-6 rounded-full bg-background border border-border-glass flex items-center justify-center z-10">
                      <div className="w-1.5 h-1.5 rounded-full bg-accent-crimson shadow-[0_0_8px_rgba(255,59,48,0.5)]" />
                    </div>
                    <div className="glass-card p-5 border-border-glass hover:border-accent-crimson/20 bg-background/50">
                      <div className="flex justify-between items-start mb-3">
                        <h3 className="text-base font-bold tracking-tight text-foreground">{exp.title}</h3>
                        <span className="text-[9px] font-mono text-muted-foreground">{new Date(exp.created_at).toLocaleDateString()}</span>
                      </div>
                      <p className="text-xs text-muted-foreground mb-5 leading-relaxed font-light">{exp.summary}</p>
                      
                      {exp.evidence_links && (
                        <div className="flex flex-wrap gap-2">
                          {exp.evidence_links.map((link: string, i: number) => (
                            <a key={i} href={link} target="_blank" className="px-2 py-1 bg-bg-glass hover:bg-bg-glass-heavy rounded border border-border-glass text-[9px] font-bold uppercase tracking-widest text-muted-foreground hover:text-foreground transition-all flex items-center gap-1.5">
                ...
                <div className="p-12 text-center text-muted-foreground font-mono text-[10px] uppercase tracking-widest border border-dashed border-border-glass rounded-3xl">
