      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
        ...
        <div className="lg:col-span-2">
          <MobileExpandableChart 
            title="Geospatial Displacements" 
            description="Interactive map of historical land seizures and forced removals."
          >
            <div className="glass-card h-[500px] sm:h-[600px] border-border-glass bg-bg-glass-heavy relative overflow-hidden group">
              <div className="absolute inset-0 flex items-center justify-center">
                ...
                {zones.map((zone, idx) => (
                  <FullScreenDataModal
                    ...
                    description={`Seizure Report - ${String(zone.location_name)}`}
                    trigger={
                      <div className="absolute w-8 h-8 -ml-4 -mt-4 cursor-pointer touch-manipulation z-10" style={{ left: `${[18, 62, 78][idx % 3]}%`, top: `${[78, 32, 48][idx % 3]}%` }}>
                        <div className="w-full h-full bg-accent-crimson/20 border-2 border-accent-crimson rounded-full animate-pulse hover:scale-125 transition-transform" />
                      </div>
                    }
                  >
              ...
              <div className="absolute bottom-6 left-6 p-4 glass-card bg-background/60 border-border-glass backdrop-blur-xl pointer-events-none">
                ...
                <FullScreenDataModal
                  ...
                  trigger={
                    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
                      className="glass-card p-5 sm:p-6 border-border-glass bg-bg-glass hover:bg-bg-glass-heavy cursor-pointer transition-all touch-manipulation group">
                      ...
                      <h4 className="text-[15px] font-bold tracking-tighter uppercase mb-2 text-foreground group-hover:text-accent-crimson transition-colors">{String(zone.title ?? '')}</h4>
...
const ZoneDetail = ({ zone }: { zone: any }) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 pt-4 pb-12">
    <div className="space-y-6">
      <div>
        ...
        <h2 className="text-3xl sm:text-4xl font-bold tracking-tighter uppercase mb-3 text-foreground">{String(zone.title ?? '')}</h2>
        <p className="text-[13px] font-mono text-muted-foreground uppercase">{String(zone.location_name ?? '')}, South Africa</p>
      </div>
      <div className="p-5 bg-bg-glass border border-border-glass rounded-2xl">
        ...
        <p className="text-[13px] text-foreground leading-relaxed font-medium italic">"{String(zone.description ?? '')}"</p>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div className="glass-card p-5 border-border-glass bg-bg-glass"><p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Seizure Date</p><p className="text-lg font-bold font-mono tracking-tighter text-foreground">{String(zone.date_of_seizure ?? 'Unknown')}</p></div>
        <div className="glass-card p-5 border-border-glass bg-bg-glass"><p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Displacement</p><p className="text-lg font-bold font-mono tracking-tighter text-accent-crimson">{Number((zone.metadata as Record<string, number>)?.families ?? 0).toLocaleString()}</p></div>
      </div>
    </div>
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center gap-4 mb-5">
          <div className="flex-1 h-2.5 bg-bg-glass-heavy rounded-full overflow-hidden"><motion.div className="h-full bg-accent-crimson" initial={{ width: 0 }} animate={{ width: "15%" }} /></div>
          <span className="text-[12px] font-mono font-bold text-accent-crimson">15% RESOLVED</span>
        </div>
        <button className="w-full py-4 bg-foreground border border-border-glass text-background text-[11px] font-bold uppercase tracking-[0.2em] rounded-2xl hover:bg-accent-crimson hover:text-white transition-all group shadow-glow-crimson">
          Sign Petition for Restoration <ArrowRight className="inline-flex w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" />
        </button>
      </div>
    </div>
  </div>
