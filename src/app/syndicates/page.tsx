  ...
  const renderNode = (node: any, level = 0) => (
    <div key={node.id} className="flex flex-col items-center gap-8 relative">
      <motion.div 
        ...
        className={`glass-card p-6 min-w-[240px] text-center cursor-pointer border-white/5 bg-white/[0.01] hover:bg-white/[0.03] transition-all group relative ${selectedNode?.id === node.id ? 'border-accent-crimson/40 bg-accent-crimson/[0.02] shadow-glow-crimson' : ''}`}
      >
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 flex items-center gap-2 px-2 py-0.5 bg-black border border-white/10 rounded text-[8px] font-mono tracking-widest uppercase text-white/40">
          {node.type}
        </div>
        <h4 className="text-sm font-bold tracking-tight uppercase mb-1">{node.name}</h4>
      ...
      <div className="container py-20 relative z-10 flex flex-col h-screen">
        <header className="mb-16 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-accent-crimson/10 rounded-2xl flex items-center justify-center border border-accent-crimson/20">
            ...
            <div>
              ...
              <h1 className="text-4xl font-bold tracking-tighter uppercase">Syndicate Structures</h1>
            </div>
          </div>

          <div className="flex gap-4">
            {SYNDICATES.map(s => (
              <button key={s.id} className="px-6 py-2.5 bg-white/5 border border-white/10 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-white/10 transition-all text-white/40 hover:text-white">
                {s.name}
              </button>
            ))}
          </div>
        ...
        {/* Hierarchy Tree Area */}
        <div className="flex-1 overflow-auto p-12 scrollbar-hide border border-white/5 rounded-3xl bg-white/[0.01]">
        ...
        {selectedNode && (
          <motion.div 
            initial={{ x: 400 }}
            animate={{ x: 0 }}
            className="absolute top-0 right-0 w-[450px] h-full glass-card border-l border-white/10 bg-black/60 backdrop-blur-2xl p-12 z-50 flex flex-col shadow-glow-crimson"
          >
            <button 
              onClick={() => setSelectedNode(null)}
              className="self-end text-[10px] font-bold uppercase tracking-widest text-white/20 hover:text-white transition-colors mb-12"
            >
              Close Dossier [ESC]
            </button>
            ...
            <div className="flex-1 space-y-12">
              <div>
                <div className="inline-flex items-center gap-2 px-2 py-0.5 bg-white/5 border border-white/10 rounded text-[9px] font-mono tracking-widest uppercase text-white/40 mb-4">
                  Node_Type_{selectedNode.type}
                </div>
                <h2 className="text-5xl font-bold tracking-tighter uppercase mb-2 leading-none">{selectedNode.name}</h2>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="glass-card p-6 border-white/5">
                  <p className="text-[9px] font-bold text-white/20 uppercase tracking-widest mb-2">Direct Links</p>
                  <p className="text-2xl font-bold font-mono tracking-tighter">{selectedNode.children?.length || 0}</p>
                </div>
                <div className="glass-card p-6 border-white/5">
                  <p className="text-[9px] font-bold text-white/20 uppercase tracking-widest mb-2">Risk Score</p>
                  <p className="text-2xl font-bold font-mono tracking-tighter text-accent-crimson">{selectedNode.risk}%</p>
                </div>
              </div>

              <div className="space-y-4">
                <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest">Infiltration Tactics</p>
                <div className="flex gap-4">
                  <div className="p-3 bg-white/5 rounded-xl border border-white/10"><Scale className="w-5 h-5 text-accent-blue" /></div>
                  <div className="p-3 bg-white/5 rounded-xl border border-white/10"><Lock className="w-5 h-5 text-accent-gold" /></div>
                  <div className="p-3 bg-white/5 rounded-xl border border-white/10"><Zap className="w-5 h-5 text-accent-crimson" /></div>
                </div>
              </div>
            </div>
