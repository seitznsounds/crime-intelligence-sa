  ...
  return (
    ...
    >
        <MobileExpandableChart 
          ...
          description="Interactive 3D visualization of crime density across South Africa."
        >
          <div className="relative w-full h-[600px] sm:h-[800px] glass-card border-border-glass bg-bg-glass-heavy rounded-3xl overflow-hidden flex items-center justify-center p-4 sm:p-10">
            ...
            >
                ...
                {hotspots.map((spot) => (
                  <g key={spot.id}>
                    <motion.circle 
                      ...
                      transition={{ duration: 4, repeat: Infinity, delay: Number(spot.id) * 0.5 }}
                    />
                    <defs>
                    ...
                    <motion.circle 
                      ...
                      className="fill-accent-crimson cursor-pointer shadow-glow-crimson pointer-events-none"
                    />
                    <circle 
                      cx={spot.x} cy={spot.y} r="25" 
                      className="fill-transparent stroke-accent-crimson/10 stroke-1 cursor-pointer hover:stroke-accent-crimson/40 transition-all touch-manipulation z-20"
              ...
              {/* Map Controls */}
              <div className="absolute bottom-6 right-6 flex flex-col gap-3 pointer-events-none sm:pointer-events-auto">
                <button className="flex items-center gap-3 px-4 py-2 bg-bg-glass border border-border-glass rounded-xl text-[11px] font-bold uppercase tracking-widest text-muted-foreground hover:text-foreground transition-all backdrop-blur-md pointer-events-auto">
                  <Zap className="w-3.5 h-3.5" /> High Precision Mode
                </button>
                <button className="flex items-center gap-3 px-4 py-2 bg-accent-crimson/10 border border-accent-crimson/20 rounded-xl text-[11px] font-bold uppercase tracking-widest text-accent-crimson transition-all backdrop-blur-md pointer-events-auto">
            ...
            <FullScreenDataModal
              ...
              title={selectedHotspot?.name || "Hotspot Dossier"}
              description={`Hotspot_ID_${selectedHotspot?.id}`}
            >
              {selectedHotspot && (
                <div className="space-y-6">
                  ...
                  <div className="p-5 bg-bg-glass border border-border-glass rounded-2xl">
                    ...
                    <div className="flex items-center justify-between gap-4 mb-4">
                      <div className="flex-1 h-2.5 bg-bg-glass-heavy rounded-full overflow-hidden">
                        <motion.div 
