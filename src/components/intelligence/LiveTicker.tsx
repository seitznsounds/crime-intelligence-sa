...
export default function LiveTicker() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-black/90 backdrop-blur-2xl border-t border-white/5 h-16 flex items-center overflow-hidden">
      {/* Live Status Indicator */}
      <div className="flex items-center gap-4 px-8 border-r border-white/10 h-full bg-accent-crimson/5">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[8px] font-mono text-white/40 uppercase tracking-widest">Latency: 42ms</span>
        </div>
      </div>
        ...
        <div className="flex items-center gap-12 px-12 whitespace-nowrap animate-marquee">
            ...
            {incidents.map((incident, i) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="text-[9px] font-mono text-white/20 uppercase tracking-widest">[{new Date(incident.created_at).toLocaleTimeString()}]</span>
                ...
                <div className="flex items-center gap-2">
                   ...
                   <span className="text-[10px] font-bold uppercase tracking-tighter text-white/80 group-hover:text-white transition-colors">{incident.location}</span>
                </div>
          ...
          {incidents.length > 0 && incidents.map((incident, i) => (
              <div key={`${incident.id}-repeat`} className="flex items-center gap-4">
                ...
                <span className="text-[10px] font-bold uppercase tracking-tighter text-white/40">{incident.location}</span>
                <div className="w-px h-4 bg-white/10" />
              </div>
            ))}
        </div>
      ...
      <div className="hidden lg:flex items-center gap-6 px-8 border-l border-white/10 h-full bg-white/[0.01]">
         <div className="flex items-center gap-3">
            <RefreshCw className={`w-3.5 h-3.5 text-white/20 ${isLive ? 'animate-spin-slow' : ''}`} />
            <div className="text-right">
               <p className="text-[8px] font-bold text-white/20 uppercase tracking-widest">Active_Sessions</p>
               <p className="text-[10px] font-mono font-bold text-accent-blue">1,204_USERS</p>
            </div>
         </div>
         <button className="p-2 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 transition-all group">
